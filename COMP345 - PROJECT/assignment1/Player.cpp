...
int Player::get_armies_deployed(Map* map)
{
	int armies_on_map = 0;
	vector<Territory*> v = map->get_territories(this);
	for (auto t : v) {
		armies_on_map += t->get_armies();
	}
	return 0;
}
...
int Player::issueOrder(int player_id, Map* map, int reinforcements, bool phaseMode) {
			...
			for (auto t : territoriesToDefend) {
				...
				if (deployments_available > 0) {
					...
					if (deploy->validate(this, t)) {
						...
						data->player = this;
						data->target = t;
						data->reinforcement = armies;
						this->orderDataVector.push_back(data);
	...
	while (true) {
		...
		if ( this->get_armies_deployed(map) <= 0 ) {
			cout << "\nYou have 0 armies on the map.\n\n";
			break;
		}
		...
		if ((territories_owned.size() >= 1)) {
			...
			//	execute specific advance order
			...
			if (advance->validate(this, territories_owned[origin], origin_neighbours[destination])) {
				...
				orderData* data = new orderData();
				data->player = this;
				data->target = origin_neighbours[destination];
				data->reinforcement = amount;
				this->orderDataVector.push_back(data);
				cout << "\n\tDeploy Order created!";
			}
			else {
	...
	if (actual_hand->size() > 0) {
		for (auto c : *actual_hand) {
			cout << "   " << c;
		}
	}
	else
		cout << " You have no cards in your hand currently\n";
